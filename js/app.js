...
function loadPageLabelVisibility() {
  try {
    ...
    return Object.prototype.hasOwnProperty.call(saved, 'includePageLabels')
      ? saved.includePageLabels === true
      : true;
  } catch {
    return true;
  }
...
function savePageLabelVisibility(visible) {
  try {
    const raw = localStorage.getItem(LUCID_SETTINGS_STORAGE_KEY);
    const saved = raw ? JSON.parse(raw) : {};
    saved.includePageLabels = visible === true;
    localStorage.setItem(LUCID_SETTINGS_STORAGE_KEY, JSON.stringify(saved));
  } catch {}
}

function loadImportSettings() {
  try {
    const saved = JSON.parse(localStorage.getItem(IMPORT_SETTINGS_STORAGE_KEY) || '{}');
    const savedStart = Number(saved.labelStart);
    return {
      labelPrefix: typeof saved.labelPrefix === 'string' ? saved.labelPrefix : 'Page',
      labelStart: Number.isFinite(savedStart) ? Math.max(0, Math.round(savedStart)) : 1,
      autoSizeCells: saved.autoSizeCells === true,
      autoSizeMode: saved.autoSizeMode === 'height' ? 'height' : 'width'
    };
  } catch {
    return { labelPrefix: 'Page', labelStart: 1, autoSizeCells: false, autoSizeMode: 'width' };
  }
}
...
function getImportSettingsFromInputs() {
  const importAutoSizeInput = document.getElementById('importAutoSizeCellsInput');
  if (importAutoSizeInput) state.autoSizeCells = importAutoSizeInput.checked;
  return {
    labelPrefix: document.getElementById('importLabelPrefixInput')?.value.trim() || '',
    labelStart: Math.max(0, Math.round(Number(document.getElementById('importLabelStartInput')?.value) || 0)),
    autoSizeCells: state.autoSizeCells === true,
    autoSizeMode: state.autoSizeMode === 'height' ? 'height' : 'width'
  };
}

function captureImportPreferences() {
  const settings = getImportSettingsFromInputs();
  importSettings.labelPrefix = settings.labelPrefix;
  importSettings.labelStart = settings.labelStart;
  importSettings.autoSizeCells = settings.autoSizeCells;
  importSettings.autoSizeMode = settings.autoSizeMode;
...
const THEME_STORAGE_KEY = 'png-grid-theme';
...
function loadThemePreference() {
  try {
    return localStorage.getItem(THEME_STORAGE_KEY) === 'light' ? 'light' : THEME_DEFAULT;
  } catch {
...
function saveThemePreference(theme) {
  try {
    localStorage.setItem(THEME_STORAGE_KEY, theme === 'light' ? 'light' : THEME_DEFAULT);
  } catch {
    // localStorage can be unavailable in some privacy contexts; ignore.
  }
}
...
function applyThemePreference(theme) {
  const normalizedTheme = theme === 'light' ? 'light' : THEME_DEFAULT;
  document.documentElement.dataset.theme = normalizedTheme;
  if (els.lightThemeInput) els.lightThemeInput.checked = normalizedTheme === 'light';
  const themeColor = document.querySelector('meta[name="theme-color"]');
  if (themeColor) themeColor.content = normalizedTheme === 'light' ? '#e8f0f1' : '#0c1320';
}
...
state.autoSizeCells = importSettings.autoSizeCells;
state.autoSizeMode = importSettings.autoSizeMode;
state.showPageLabels = loadPageLabelVisibility();
...
function captureStateSnapshot(label = 'Action') {
  const snapshot = {
    ...
    state: {
      ...
      linkedColumnGaps: state.linkedColumnGaps.slice(),
      gapX: state.gapX,
      gapY: state.gapY,
...
function restoreStateSnapshot(snapshot) {
  ...
  state.linkedColumnGaps = Array.isArray(s.linkedColumnGaps)
    ? s.linkedColumnGaps.map(Boolean)
    : state.columnGaps.map(() => false);
...
const GAP_PIXELS_PER_LEVEL = 2;

function gapLevelToPixels(level) {
  const numericLevel = Number(level);
  return Number.isFinite(numericLevel)
    ? Math.max(0, numericLevel) * GAP_PIXELS_PER_LEVEL
    : 0;
}

function getActualGlobalGapX() {
  return gapLevelToPixels(state.globalGapX);
}

function getActualGapY() {
  return gapLevelToPixels(state.gapY);
}
...
function getSizeLabel(type, value) {
  const sizes = {
    gapX: [
      ...
      { threshold: 15, label: 'xtra-small' },
      { threshold: 25, label: 'small' },
      { threshold: 35, label: 'medium' },
      { threshold: 45, label: 'xtra-medium' },
      { threshold: 55, label: 'large' },
      { threshold: 65, label: 'xtra-large' }
    ],
    gapY: [
      { threshold: 5, label: 'compact' },
      { threshold: 15, label: 'xtra-small' },
    ...
    cellWidth: [
      ...
      { threshold: 110, label: 'xtra-small' },
      { threshold: 140, label: 'small' },
      { threshold: 200, label: 'medium' },
      { threshold: 350, label: 'xtra-medium' },
      { threshold: 600, label: 'large' },
      { threshold: CELL_SIZE_MAX, label: 'xtra-large' }
    ],
    cellHeight: [
      { threshold: 70, label: 'compact' },
      { threshold: 90, label: 'xtra-small' },
      { threshold: 110, label: 'small' },
      { threshold: 160, label: 'medium' },
      { threshold: 320, label: 'xtra-medium' },
      { threshold: 600, label: 'large' },
      { threshold: CELL_SIZE_MAX, label: 'xtra-large' }
    ],
    textSize: [
...
function normalizeColumnGaps({ reset = false } = {}) {
  ...
  const rawFallback = Number(state.globalGapX ?? state.gapX ?? 12);
  const fallback = clamp(Number.isFinite(rawFallback) ? rawFallback : 12, 0, 120);
  state.globalGapX = fallback;
  state.gapX = fallback;

  const source = Array.isArray(state.columnGaps) ? state.columnGaps : [];
  const linkedSource = Array.isArray(state.linkedColumnGaps) ? state.linkedColumnGaps : [];
  const next = new Array(gapCount);
  const nextLinked = new Array(gapCount).fill(false);
  for (let i = 0; i < gapCount; i += 1) {
    ...
    nextLinked[i] = linkedSource[i] === true;
  }
  state.columnGaps = next;
  state.linkedColumnGaps = nextLinked;
}

function isColumnGapLinked(colIndex) {
  return Number.isInteger(colIndex) && state.linkedColumnGaps?.[colIndex] === true;
}
...
function getGapAfterColumn(colIndex) {
  ...
  return isColumnGapLinked(colIndex)
    ? 0
    : (Number.isFinite(gap) ? gapLevelToPixels(gap) : getActualGlobalGapX());
}
...
function getLayoutMetrics() {
  ...
  const innerWidth = state.cols > 0
    ...
    : cellWidth;
  const innerHeight = cellHeight * state.rows + getActualGapY() * Math.max(0, state.rows - 1);
...
function resizeGridWithDirectionalExpansion({ left = 0, right = 0, top = 0, bottom = 0 }) {
  ...
  const colStride = metrics.cellWidth + getActualGlobalGapX();
  const rowStride = metrics.cellHeight + getActualGapY();
  ...
  const previousLinkedGaps = state.linkedColumnGaps.slice();
  state.rows = nextRows;
  state.cols = nextCols;
  if (actualColsAdded > 0) {
    ...
    const leftLinked = new Array(appliedLeft).fill(false);
    const rightLinked = new Array(appliedRight).fill(false);
    state.linkedColumnGaps = leftLinked.concat(previousLinkedGaps, rightLinked);
  }
  normalizeColumnGaps();
  state.grid = newGrid;
...
function shrinkGridWithDirectionalReduction({ left = 0, right = 0, top = 0, bottom = 0 }) {
  ...
  const nextLinkedGaps = state.linkedColumnGaps.slice();
  if (appliedLeft > 0) {
    nextGaps.splice(0, appliedLeft);
  }
  if (appliedRight > 0) {
    nextGaps.splice(Math.max(0, nextGaps.length - appliedRight), appliedRight);
    nextLinkedGaps.splice(Math.max(0, nextLinkedGaps.length - appliedRight), appliedRight);
  }
  if (appliedLeft > 0) nextLinkedGaps.splice(0, appliedLeft);

  state.rows = nextRows;
  state.cols = nextCols;
  state.columnGaps = nextGaps;
  state.linkedColumnGaps = nextLinkedGaps;
  normalizeColumnGaps();
  state.grid = newGrid;
...
function persistSession() {
  const payload = {
    ...
    linkedColumnGaps: state.linkedColumnGaps,
    gapX: state.gapX,
    gapY: state.gapY,
...
async function restoreSession() {
  try {
    ...
    state.linkedColumnGaps = Array.isArray(saved.linkedColumnGaps)
      ? saved.linkedColumnGaps.map(Boolean)
      : state.columnGaps.map(() => false);
    normalizeColumnGaps();
    state.gapY = clamp(Number(saved.gapY ?? legacyGap), 0, 120);
    state.cellWidth = clamp(Number(saved.cellWidth || 160), CELL_SIZE_MIN, CELL_SIZE_MAX);
    state.cellHeight = clamp(Number(saved.cellHeight || 120), CELL_SIZE_MIN, CELL_SIZE_MAX);
...
function syncSettingsInputs() {
  ...
  updateSizeLabel('gapXInput', 'gapXLabel');
  updateSizeLabel('gapYInput', 'gapYLabel');
  updateSizeLabel('cellWidthInput', 'cellWidthLabel');
  updateSizeLabel('cellHeightInput', 'cellHeightLabel');
  if (els.autoSizeCellsInput) els.autoSizeCellsInput.checked = state.autoSizeCells === true;
  if (els.autoSizeModeSelect) els.autoSizeModeSelect.value = state.autoSizeMode === 'height' ? 'height' : 'width';
  if (els.autoSizeModeField) els.autoSizeModeField.hidden = state.autoSizeCells !== true;
  state.fit = 'contain';
...
function applyZoomSize() {
  if (!els.grid) return;
  const metrics = getLayoutMetrics();
  const canvasZoom = Math.max(0.0001, Number(state.zoom) || 1);
  els.grid.style.width = `${metrics.width * canvasZoom}px`;
  els.grid.style.height = `${metrics.height * canvasZoom}px`;
  els.grid.style.setProperty('--canvas-zoom', String(canvasZoom));
  els.grid.classList.toggle('grid-captions-collapsed', metrics.cellHeight * canvasZoom < 42);
}
...
function applyAutoSizeToAssets(assets) {
  if (!state.autoSizeCells || !assets?.length) return false;
  const dimension = state.autoSizeMode === 'height' ? 'height' : 'width';
  const sizedAssets = assets.filter(asset => (
    Number.isFinite(Number(asset.width)) && Number(asset.width) > 0 &&
    Number.isFinite(Number(asset.height)) && Number(asset.height) > 0
  ));
  if (sizedAssets.length === 0) return false;

  // The grid uses one shared cell rectangle. Use the image with the largest
  // selected dimension as the reference so both cell dimensions keep its
  // intrinsic portrait or landscape aspect ratio.
  const reference = sizedAssets.reduce((largest, asset) => (
    Number(asset[dimension]) > Number(largest[dimension]) ? asset : largest
  ));
  const rawWidth = Math.max(1, Math.round(Number(reference.width)) + 2);
  const rawHeight = Math.max(1, Math.round(Number(reference.height)) + 2);
  const minScale = Math.max(CELL_SIZE_MIN / rawWidth, CELL_SIZE_MIN / rawHeight);
  const maxScale = Math.min(CELL_SIZE_MAX / rawWidth, CELL_SIZE_MAX / rawHeight);
  // Keep the image ratio intact. For extreme aspect ratios where the minimum
  // and maximum bounds cannot both be satisfied, prioritize the max bound.
  const scale = minScale <= maxScale
    ? clamp(1, minScale, maxScale)
    : maxScale;
  const nextWidth = Math.max(1, Math.round(rawWidth * scale));
  const nextHeight = Math.max(1, Math.round(rawHeight * scale));
  const changed = state.cellWidth !== nextWidth || state.cellHeight !== nextHeight;
  state.cellWidth = nextWidth;
  state.cellHeight = nextHeight;
  return changed;
}

async function refreshAutoSizeForCurrentAssets() {
  if (!state.autoSizeCells || state.assets.length === 0) {
    return { changed: false, hasDimensions: false };
  }

  await Promise.all(state.assets.map(async asset => {
    const hasDimensions = Number.isFinite(Number(asset.width)) && asset.width > 0
      && Number.isFinite(Number(asset.height)) && asset.height > 0;

    if (hasDimensions) return;

    const sources = [asset.thumbUrl, asset.dataUrl, asset.src, asset.url]
      .filter(source => typeof source === 'string' && source.length > 0);
    for (const source of sources) {
      try {
        const image = await loadImageOnce(source);
        ...
        // Try the next legacy source or the original stored in IndexedDB.
      }
    }

    if (!asset.id) return;
    const blob = await readAssetBlob(asset.id);
    if (!blob) return;
    const objectUrl = URL.createObjectURL(blob);
    try {
      const image = await loadImageOnce(objectUrl);
      asset.width = image.naturalWidth;
      asset.height = image.naturalHeight;
    } catch {
      // Leave assets without readable previews unchanged.
    } finally {
      URL.revokeObjectURL(objectUrl);
    }
  }));

  const hasDimensions = state.assets.some(asset => (
    Number.isFinite(Number(asset.width)) && Number(asset.width) > 0 &&
    Number.isFinite(Number(asset.height)) && Number(asset.height) > 0
  ));
  return { changed: applyAutoSizeToAssets(state.assets), hasDimensions };
}

function getAssetGridLabel(asset) {
  if (!state.showPageLabels) return '';
  const assetIndex = state.assets.indexOf(asset);
  if (assetIndex >= 0 && importSettings.labelPrefix) {
    return `${importSettings.labelPrefix} ${importSettings.labelStart + assetIndex}`;
  }
  if (asset?.pageLabel) return asset.pageLabel;
  return getDisplayName(asset?.name);
}

function applyImportLabelsToCurrentAssets() {
  const prefix = String(importSettings.labelPrefix || '').trim();
  const start = Math.max(0, Math.round(Number(importSettings.labelStart) || 0));
  state.assets.forEach((asset, index) => {
    asset.pageLabel = prefix ? `${prefix} ${start + index}` : '';
  });
}

function applyPageLabelDefaultsFromSettingsInputs() {
  const labelPrefix = els.importSettingsLabelPrefixInput?.value.trim() || '';
  const labelStart = Math.max(0, Math.round(Number(els.importSettingsLabelStartInput?.value) || 0));
  importSettings.labelPrefix = labelPrefix;
  importSettings.labelStart = labelStart;
  saveImportSettings({ ...importSettings });
  applyImportLabelsToCurrentAssets();
}
...
function placeAssetInRowFlow(assetId, targetRow, insertCol) {
  ...
  if (newRowItems.length > oldCols) {
    ...
    state.canvasWidth = Math.max(1, state.canvasWidth + addCols * (state.cellWidth + getActualGlobalGapX()));
  } else {
    state.grid = tempGrid;
  }
...
function renderGapControls() {
  ...
  for (let i = 0; i < state.columnGaps.length; i += 1) {
    ...
    const isLinked = isColumnGapLinked(i);
    button.type = 'button';
    ...
    button.textContent = isLinked
      ? `Spread: Col. ${i + 1} | Col. ${i + 2}`
      : `Join Col. ${i + 1} & ${i + 2} together`;
    button.addEventListener('pointerenter', () => showSpreadPreview(i));
    button.addEventListener('pointerleave', () => clearSpreadPreview());
    button.addEventListener('focus', () => showSpreadPreview(i));
    button.addEventListener('blur', () => clearSpreadPreview());
...
function showSpreadPreview(gapIndex) {
  if (!els.grid || !Number.isInteger(gapIndex) || gapIndex < 0 || gapIndex >= state.cols - 1) return;
  clearSpreadPreview();
  els.grid.dataset.spreadPreview = `${gapIndex + 1}-${gapIndex + 2}`;
  els.grid.querySelectorAll('.grid-cell').forEach(cell => {
    const index = Number(cell.dataset.index);
    const col = index % state.cols;
    if (col === gapIndex || col === gapIndex + 1) {
      cell.classList.add(col === gapIndex ? 'spread-preview-left' : 'spread-preview-right');
      if (Math.floor(index / state.cols) === 0) {
        cell.classList.add(col === gapIndex ? 'spread-preview-label-left' : 'spread-preview-label-right');
        cell.dataset.columnLabel = `COL ${col + 1}`;
      }
    }
  });
}

function clearSpreadPreview() {
  if (!els.grid) return;
  delete els.grid.dataset.spreadPreview;
  els.grid.querySelectorAll('.spread-preview-left, .spread-preview-right, .spread-preview-label-left, .spread-preview-label-right')
    .forEach(cell => cell.classList.remove(
      'spread-preview-left',
      'spread-preview-right',
      'spread-preview-label-left',
      'spread-preview-label-right'
    ));
}
...
function closeCellActionMenus(except = null) {
  ...
  function updateInsertPreview(targetIndex, placement) {
    ...
    } else if (isColumnGapLinked(insertCol - 1)) {
    ...
    const rowY = row * (metrics.cellHeight + getActualGapY());
...
function resolveFlowInsertionForGap(event) {
  ...
  const gapY = getActualGapY() * scaleY;
...
function removeRowAt(rowIndex) {
  ...
  state.rows = nextRows;
  state.grid = nextGrid;
  state.canvasHeight = Math.max(1, state.canvasHeight - (metrics.cellHeight + getActualGapY()));
  state.selectedSlotIndex = mapIndex(state.selectedSlotIndex);
  if (state.previewModalOpen) {
    state.previewSlotIndex = mapIndex(state.previewSlotIndex);
  }
...
function removeColumnAt(colIndex) {
  ...
  if (nextGaps.length > 0) {
    ...
    nextLinkedGaps.splice(removeGapAt, 1);
  }
  ...
  normalizeColumnGaps();
  state.grid = nextGrid;
  state.canvasWidth = Math.max(1, state.canvasWidth - (metrics.cellWidth + getActualGlobalGapX()));
  state.selectedSlotIndex = mapIndex(state.selectedSlotIndex);
  if (state.previewModalOpen) {
    state.previewSlotIndex = mapIndex(state.previewSlotIndex);
  }
...
function insertColumnAt(insertAt) {
  ...
  const oldLinkedGaps = state.linkedColumnGaps.slice();
  const newGaps = [];
  const newLinkedGaps = [];
  ...
  state.linkedColumnGaps = newLinkedGaps;
  normalizeColumnGaps();
  state.canvasWidth = Math.max(1, state.canvasWidth + state.cellWidth + getActualGlobalGapX());
...
function insertRowAt(insertAt) {
  ...
  state.rows = newRows;
  state.grid = newGrid;
  state.canvasHeight = Math.max(1, state.canvasHeight + state.cellHeight + getActualGapY());
...
function beginAutoExpandSession() {
  ...
  const metrics = getLayoutMetrics();
  const strideX = Math.max(1, (metrics.cellWidth + getActualGlobalGapX()) * state.zoom);
  const strideY = Math.max(1, (metrics.cellHeight + getActualGapY()) * state.zoom);
...
function getPreviewSpreadGroup(slotIndex) {
  ...
  while (startCol > 0) {
    ...
    if (!isColumnGapLinked(startCol - 1)) break;
  ...
  while (endCol < state.cols - 1) {
    ...
    if (!isColumnGapLinked(endCol)) break;
...
function getPreviewSpreadRenderScale(sortedSlots, metrics, { fullRes = false } = {}) {
  ...
  const baseWidth = Math.max(1, metrics.cellWidth * sortedSlots.length);
...
async function renderPreviewGroupDataUrl(slotIndices, { fullRes = false } = {}) {
  ...
  const scaledHeight = Math.max(1, Math.round(metrics.cellHeight * renderScale));
  ...
  try {
    // Scale every image to the same height and derive its natural width — no slots, no gaps.
    const frames = await Promise.all(sorted.map(async slotIndex => {
      const assetId = state.grid[slotIndex];
      const asset = assetId ? findAssetById(assetId) : null;
      const fallbackW = Math.round(metrics.cellWidth * renderScale);
      if (!asset) return { image: null, w: fallbackW };
      const src = fullRes ? (fullResUrls?.get(assetId) || asset.thumbUrl) : asset.thumbUrl;
      const image = fullRes ? await loadImageOnce(src) : await loadImage(src);
      const naturalW = image.naturalWidth || image.width || metrics.cellWidth;
      const naturalH = image.naturalHeight || image.height || metrics.cellHeight;
      const w = Math.max(1, Math.round(scaledHeight * naturalW / Math.max(1, naturalH)));
      return { image, w };
    }));

    const totalWidth = frames.reduce((s, f) => s + f.w, 0);
    const canvas = document.createElement('canvas');
    ...
    let x = 0;
    for (const { image, w } of frames) {
      if (image) ctx.drawImage(image, x, 0, w, scaledHeight);
      x += w;
    }
...
async function syncPreviewModal() {
  ...
  const previewLabel = getAssetGridLabel(firstAsset) || firstAsset.name;
  els.previewModalTitle.textContent = isSpread ? `Spread preview (${group.length} images)` : previewLabel;
...
function createGridEdgeButtons(metrics) {
  ...
  for (let boundary = 0; boundary <= state.rows; boundary += 1) {
    ...
    if (boundary === 0) {
      ...
      const prevBottom = metrics.offsetY + (boundary - 1) * (metrics.cellHeight + getActualGapY()) + metrics.cellHeight;
      const nextTop = metrics.offsetY + boundary * (metrics.cellHeight + getActualGapY());
  ...
  if (state.rows > 1) {
    for (let row = 0; row < state.rows; row += 1) {
      const y = metrics.offsetY + row * (metrics.cellHeight + getActualGapY()) + metrics.cellHeight / 2;
...
function createGridCell(assetId, index, frame) {
  ...
  const col = index % state.cols;
  if (isColumnGapLinked(col)) {
    cell.classList.add('spread-left');
  }
  if (col > 0 && isColumnGapLinked(col - 1)) {
  ...
  if (assetId) {
    ...
    if (asset) {
      ...
      caption.textContent = getAssetGridLabel(asset);
      caption.title = asset.name;
      if (caption.textContent) slotButton.appendChild(caption);
    }
  }
  ...
  const activateCell = async event => {
    ...
    if (state.awaitingAppendSelection && state.pendingImportFiles?.length) {
      ...
      const options = state.pendingImportOptions || captureImportPreferences();
      state.pendingImportFiles = null;
      state.pendingImportOptions = null;
      state.awaitingAppendSelection = false;
      await executeImportMode(files, 'append-selected', index, options);
      return;
    }
    renderGrid();
    focusAfterRender(index);
...
function renderGrid() {
  ...
  for (let index = 0; index < state.grid.length; index += 1) {
    ...
    const y = metrics.offsetY + row * (metrics.cellHeight + getActualGapY());
    const frame = {
...
function inferLucidPageLabel(asset, fallbackSlotIndex) {
  if (asset?.pageLabel) return String(asset.pageLabel);
...
function getSpreadEndColForLucid(row, startCol) {
  ...
  for (let col = startCol; col < state.cols - 1; col += 1) {
    ...
    if (!isColumnGapLinked(col)) break;
...
function clearGrid() {
  ...
  state.linkedColumnGaps = [false, false];
  state.gapY = 12;
  state.selectedSlotIndex = null;
  state.pendingImportFiles = null;
...
function applyNumberSettings() {
  ...
  if (state.globalGapX !== prevGlobalGapX) {
    state.columnGaps = state.columnGaps.map((gap, index) => (
      isColumnGapLinked(index) ? 0 : state.globalGapX
    ));
  }
  normalizeColumnGaps();
  state.gapY = clamp(Number(els.gapYInput.value || 0), 0, 120);
  state.cellWidth = clamp(Number(els.cellWidthInput.value || 160), CELL_SIZE_MIN, CELL_SIZE_MAX);
  state.cellHeight = clamp(Number(els.cellHeightInput.value || 120), CELL_SIZE_MIN, CELL_SIZE_MAX);
  state.fit = 'contain';
...
function openImportModeModal(fileCount) {
  ...
  if (existingCount > 0) {
    els.importExistingNotice.textContent = `Current session already has ${existingCount} image${existingCount === 1 ? '' : 's'}. Choose whether to replace, append, or stage the new images.`;
  } else {
    els.importExistingNotice.textContent = 'Current session is empty. Replace starts a fresh grid.';
  }
  if (els.importLabelPrefixInput) els.importLabelPrefixInput.value = importSettings.labelPrefix;
  if (els.importLabelStartInput) els.importLabelStartInput.value = String(importSettings.labelStart);
  if (els.importAutoSizeCellsInput) els.importAutoSizeCellsInput.checked = state.autoSizeCells === true;
...
async function executeImportMode(files, mode, selectedIndex = state.selectedSlotIndex, options = {}) {
  ...
  applyAutoSizeToAssets(nextAssets);
...
async function processDocFile(file) {
  ...
  try {
    ...
    } else if (err.code === 'DOC_LIBRARY_TIMEOUT' || err.message?.includes('CDN') || err.message?.includes('Failed to load')) {
      showToast(err.message || 'Could not load parsing library. Check your internet connection and try again.', 12000);
    } else {
...
function bindEvents() {
  ...
  els.gapControlsContainer?.addEventListener('click', event => {
    ...
    const wasLinked = isColumnGapLinked(gapIndex);
    pushHistory(`${wasLinked ? 'Unlink' : 'Link'} columns ${gapIndex + 1}-${gapIndex + 2}`);
    state.linkedColumnGaps[gapIndex] = !wasLinked;
  ...
  els.autoSizeCellsInput?.addEventListener('change', async () => {
    state.autoSizeCells = els.autoSizeCellsInput.checked;
    saveImportSettings({ ...importSettings, autoSizeCells: state.autoSizeCells });
    const result = await refreshAutoSizeForCurrentAssets();
    if (result.changed) {
      pushHistory(`Auto-size cells to full ${state.autoSizeMode}`);
      showToast(`Cells sized to full ${state.autoSizeMode}`);
    } else if (state.autoSizeCells && state.assets.length > 0) {
      showToast(result.hasDimensions
        ? `Cells already sized to full ${state.autoSizeMode}`
        : 'Could not determine the imported image dimensions');
    }
    renderAll();
  });
  els.autoSizeModeSelect?.addEventListener('change', async () => {
    state.autoSizeMode = els.autoSizeModeSelect.value === 'height' ? 'height' : 'width';
    saveImportSettings({ ...importSettings, autoSizeMode: state.autoSizeMode });
    if (state.autoSizeCells) {
      const result = await refreshAutoSizeForCurrentAssets();
      if (result.changed) {
        pushHistory(`Auto-size cells to full ${state.autoSizeMode}`);
        showToast(`Cells sized to full ${state.autoSizeMode}`);
      } else if (state.assets.length > 0 && !result.hasDimensions) {
        showToast('Could not determine the imported image dimensions');
      }
      renderAll();
    }
  });
  ...
  els.importReplaceBtn.addEventListener('click', () => {
    const files = state.pendingImportFiles || [];
    state.pendingImportOptions = captureImportPreferences();
    closeImportModeModal();
  ...
  els.replaceApplyBtn?.addEventListener('click', async () => {
    ...
    state.pendingReplaceFiles = null;
    state.pendingImportFiles = null;
    await executeImportMode(files, 'replace', state.selectedSlotIndex, {
      ...state.pendingImportOptions,
      firstRowOffset, replaceSizing, customRows, customCols
    });
    state.pendingImportOptions = null;
  });
  ...
  els.importAppendStartBtn.addEventListener('click', async () => {
    const files = state.pendingImportFiles || [];
    const options = captureImportPreferences();
    closeImportModeModal();
    state.pendingImportFiles = null;
    await executeImportMode(files, 'append-start', state.selectedSlotIndex, options);
  });
  ...
  els.importAppendEndBtn?.addEventListener('click', async () => {
    ...
    closeImportModeModal();
    state.pendingImportFiles = null;
    await executeImportMode(files, 'append-end', state.selectedSlotIndex, options);
  });
  ...
  els.importTrayBtn?.addEventListener('click', async () => {
    ...
    closeImportModeModal();
    state.pendingImportFiles = null;
    await executeImportMode(files, 'tray', state.selectedSlotIndex, options);
  });
  ...
  [els.rowsUpBtn, els.rowsDownBtn, els.colsUpBtn, els.colsDownBtn].forEach(button => {
    button?.classList.add('stepper-control');
    button?.addEventListener('pointerenter', () => {
      button.classList.add('is-pointer-hovered');
      const active = document.activeElement;
      if (active instanceof HTMLElement && active !== button && active.closest('.stepper-row')) {
        active.blur();
      }
    });
    button?.addEventListener('pointerleave', () => {
      button.classList.remove('is-pointer-hovered');
    });
    ...
    if (el) {
      ...
      el.addEventListener('input', () => {
        ...
        if (stateKey === 'gapX') {
          ...
          if ((stateKey === 'cellWidth' || stateKey === 'cellHeight') && state.autoSizeCells) {
            state.autoSizeCells = false;
            importSettings.autoSizeCells = false;
            if (els.autoSizeCellsInput) els.autoSizeCellsInput.checked = false;
            saveImportSettings({ ...importSettings });
          }
      ...
      const result = await sendGridToLucid(
        {
          ...
          cellWidth: state.cellWidth,
          cellHeight: state.cellHeight,
          gapX: getActualGlobalGapX(),
          columnGaps: state.columnGaps.map(gapLevelToPixels),
          gapY: getActualGapY(),
  ...
  // Apply interface size when the settings form is saved, not while the user
  // is still typing a numeric value.
  const applyUiScalePreference = () => {
  ...
  const applyThemeInput = () => {
    state.themePreference = els.lightThemeInput?.checked ? 'light' : THEME_DEFAULT;
    saveThemePreference(state.themePreference);
    applyThemePreference(state.themePreference);
  };
  els.lightThemeInput?.addEventListener('change', applyThemeInput);
  ...
  function openLucidSettings() {
    ...
    if (els.lightThemeInput) els.lightThemeInput.checked = state.themePreference === 'light';
    if (els.importSettingsLabelPrefixInput) els.importSettingsLabelPrefixInput.value = importSettings.labelPrefix;
    if (els.importSettingsLabelStartInput) els.importSettingsLabelStartInput.value = String(importSettings.labelStart);
    ...
    if (els.lucidIncludePageLabelsInput) els.lucidIncludePageLabelsInput.checked = state.showPageLabels === true;
  ...
  els.lucidSettingsSaveBtn?.addEventListener('click', () => {
    applyPageLabelDefaultsFromSettingsInputs();
    renderAll();

    const { saveLucidSettings } = window.__lucidExport || {};
    if (!saveLucidSettings) {
      closeLucidSettings();
      showToast('Import and interface settings saved.');
      return;
    }
...
function initElements() {
  ...
  els.autoSizeCellsInput = document.getElementById('autoSizeCellsInput');
  els.autoSizeModeField = document.getElementById('autoSizeModeField');
  els.autoSizeModeSelect = document.getElementById('autoSizeModeSelect');
  ...
  els.importLabelPrefixInput = document.getElementById('importLabelPrefixInput');
  els.importLabelStartInput = document.getElementById('importLabelStartInput');
  els.importAutoSizeCellsInput = document.getElementById('importAutoSizeCellsInput');
  ...
  els.lightThemeInput = document.getElementById('lightThemeInput');
  ...
  els.importSettingsLabelPrefixInput = document.getElementById('importSettingsLabelPrefixInput');
  els.importSettingsLabelStartInput = document.getElementById('importSettingsLabelStartInput');
...
async function init() {
  ...
  if (state.autoSizeCells) await refreshAutoSizeForCurrentAssets();
  ensureGridShape();
  bindEvents();
  bindDocImportEvents();
